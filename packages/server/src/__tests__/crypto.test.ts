...
import { sanitizeContent, sanitizeMessage, isMetadataWithinLimits, stripHtml } from '../sanitize.ts';
import {
  addMessage,
  getMessages,
...
async function signPayload(privateKey: CryptoKey, payload: string): Promise<string> {
  ...
  const signature = await crypto.subtle.sign(SIGN_ALGORITHM, privateKey, payloadBytes);
...
Deno.test('Signature creation and verification', async () => {
  ...
  const valid = await verifyRequestSignature(publicKeyJwk, signature, timestamp, nonce, path, bodyHash);
...
Deno.test('Signature verification fails with wrong key', async () => {
  ...
  const valid = await verifyRequestSignature(wrongPublicKey, signature, timestamp, nonce, path, bodyHash);
...
Deno.test('Signature verification fails with tampered path', async () => {
  ...
  const valid = await verifyRequestSignature(publicKeyJwk, signature, timestamp, nonce, '/reply', bodyHash);
  assertEquals(valid, false);
});
