...
function startMockTelegram(port: number): void {
  telegramRequests = [];
  mockTelegramServer = Deno.serve({ port, onListen: () => {} }, async (req: Request) => {
...
Deno.test({
  ...
  async fn(t) {
    ...
    try {
      ...
      await t.step('POST /channels/telegram/register without botToken returns 400', async () => {
      ...
      await t.step('POST /channels/telegram/register without auth returns 401', async () => {
      ...
      await t.step('POST /telegram/:channelId for unknown channel returns 404', async () => {
        const resp = await fetch(`${BASE}/telegram/nonexistent-channel?secret=foo`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            update_id: 1,
      ...
      await t.step('register a telegram channel via generic endpoint (for testing)', async () => {
      ...
      await t.step('POST /telegram/:channelId with wrong secret returns 401', async () => {
        const resp = await fetch(`${BASE}/telegram/${channelId}?secret=wrong-secret`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            update_id: 2,
      ...
      await t.step('POST /telegram/:channelId with valid secret ingests message', async () => {
        const resp = await fetch(`${BASE}/telegram/${channelId}?secret=${webhookSecret}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            update_id: 3,
      ...
      await t.step('GET /messages returns the Telegram message', async () => {
        ...
        assertEquals(resp.status, 200);
        const body = await resp.json();
        const telegramMsgs = body.messages.filter((m: { channelType: string }) => m.channelType === 'telegram');
      ...
      await t.step('POST /telegram/:channelId handles edited messages', async () => {
        const resp = await fetch(`${BASE}/telegram/${channelId}?secret=${webhookSecret}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
      ...
      await t.step('POST /telegram/:channelId handles callback queries', async () => {
        const resp = await fetch(`${BASE}/telegram/${channelId}?secret=${webhookSecret}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
      ...
      await t.step('POST /telegram/:channelId ignores updates without text', async () => {
        const resp = await fetch(`${BASE}/telegram/${channelId}?secret=${webhookSecret}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
      ...
      await t.step('POST /telegram/:channelId rejects invalid JSON', async () => {
        const resp = await fetch(`${BASE}/telegram/${channelId}?secret=${webhookSecret}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: 'not valid json',
      ...
      await t.step('POST /reply for telegram channel stores response', async () => {
      ...
      await t.step('GET /responses/:channelId returns telegram responses', async () => {
        const resp = await fetch(`${BASE}/responses/${channelId}`);
        assertEquals(resp.status, 200);
        const body = await resp.json();
        const telegramResps = body.responses.filter((r: { channelType: string }) => r.channelType === 'telegram');
